...
- 足球与 CS2 首页统一为「主队 / 今日 / 明日 / 最近」四个入口，今日默认，直播与未开赛优先，移除抢占首屏的赛事目录与长说明
- 比赛改为按日期分组的紧凑单列；导航与页面换成足球/准星矢量图标，缺失队徽显示稳定文字徽标
- 资源整合页赛事汇总去掉英文眉题、长说明和数据源尾注，只保留日期切换、足球/CS2 对阵与“全部”入口
- 停止把赛程临时队徽批量写入 IndexedDB；请求内存缓存限制为 40 项
- 足球「最近」7 天合并为一次 `from/to` 请求，不再连续消耗 7 次 API-Football 免费额度
...
> 根因复盘：V1.26 只扩展了 CS2 数据接口，没有在「资源整合」父级页面渲染任何赛事内容；同时 CS 首页的赛事列表与日期列表会并发请求同一个 Liquipedia URL，客户端缓存只存已完成响应，无法合并进行中的请求，首次缓存未命中时可能触发上游 `action=parse` 限流并双双失败。右滑则只在 `touchend` 判断，移动浏览器可能先取消/接管横向手势；上轮测试只覆盖纯函数，没有验证真实触摸事件。

**Added**

- 资源整合页新增「赛事汇总」：足球与 CS2 同屏，支持今天/明天/后天切换，各自展示最多 6 场并可进入完整赛事中心
- 汇总区显示主源、降级/缓存状态和数据署名，不再把“接口存在”当成用户可见功能

**Fixed**

- `rawInflight` 合并同 URL 并发读取，CS 赛事发现、日期列表与资源汇总复用同一个进行中请求，避免 Liquipedia 首次加载被并发限流
- 手机右滑改为 Pointer Events：横向移动达到 56px 后即时触发，并配合 `touch-action:pan-y` 保留纵向滚动、抑制浏览器接管横滑；从页面中心起滑也可打开侧栏
- 队徽缓存从逐场串行读写改为并行，完整足球赛程不再长时间停留在骨架屏
- 修正常用联赛映射：API-Football 的中超 ID 使用 169，不再把 ID 197 的希腊 Super League 1 错标为中超

#### 实测

- `node --check app.js` / `src/features/sports.js` / `functions/_services/football-client.js` 与 `python -m py_compile server.py`：通过
- `bash tests/run-all.sh`：16 套单元/集成测试全部通过（新增同 URL 并发合并与中超 ID 回归断言）
- 线上接口探测：足球当日接口返回 27 场；CS2 v1 全量窗口返回 92 场，无需新增 API
- 390×844 手机视口：从页面中心右滑可见左侧主菜单；资源整合页同时出现真实足球/CS2 赛程；足球完整中心 27 张赛程卡约 3.5 秒完成且骨架归零

### V1.26.0 分级导航、全屏右滑与赛事覆盖升级（2026-09-20）

> 根因：浏览器历史记录只表示访问顺序，不能表达产品层级，导致从时间线或赛事页返回时路径冗长；侧栏手势仅允许从屏幕左侧 40px 起滑；CS2 使用 Liquipedia 默认 50 场 ticker 并套 A 级白名单，TheSportsDB 旧免费 key 也已过时，造成赛事覆盖不完整。

**Added**

- 新增 `src/features/navigation.js`：集中维护页面父级、记忆/资源整合 Hub 和移动端横滑判定；时间线等记忆子页固定返回「记忆」，CS/足球等资源子页固定返回「资源整合」
- 主内容非交互区域支持从任意位置右滑呼出侧栏；侧栏打开时左滑关闭，并排除输入框、相册、横向标签和弹窗等冲突区域
- 新增 ARCH-017 CS2 链路：`LiquipediaAdapter → SportsService → /api/v1/sports/cs2/matches`，统一响应信封、稳定错误码、来源许可和覆盖元数据
- `AGENTS.md` 新增网页开发可靠性守则：显式产品层级、移动端视口验收、模块边界、官方数据合规、降级真实性、安全与完成定义

**Changed**

- CS2 通过 Liquipedia 官方 MediaWiki `action=parse` 单次展开最多 200 场赛程，取消 A 级赛事白名单；15 分钟服务端缓存，UI 展示 Liquipedia / CC BY-SA 署名
- TheSportsDB 兼容降级源从失效的公开 key `3` 更新为官方免费 key `123`；足球主链仍为 API-Football，未配置密钥或超额时继续如实显示有限降级
- `server.py` 同步实现 CS2 v1 本地路由，保证本地预览和 Cloudflare 线上行为一致

**Fixed**

- 修复浏览器/手机系统返回在页面访问历史中逐项倒退，而不是回到产品父级的问题
- 修复 CS2 赛事发现只显示少量 A 级赛事、关注队部分比赛只能在特殊路径中找到的问题

#### 实测

- `node --check`：`app.js`、导航/体育 feature、Liquipedia Adapter、Sports Service 和 v1 route 全部通过
- `python -m py_compile server.py` 通过
- 新增导航父级/横滑单测与 CS2 Adapter→Service→v1 路由单测；`bash tests/run-all.sh`：16 套单元/集成测试全部通过
- 真实浏览器 375×812、390×844、430×932 验收：记忆/资源 Hub 均正常；时间线系统返回到记忆、CS 系统返回和页内返回均到资源整合；三个宽度横向溢出均为 0；控制台无错误

### V1.25.0 手机端时间线、相册与自定义皮肤（2026-09-20）

> 根因：手机时间线卡片只设左外边距，没有扣减自身宽度，窄屏会产生横向溢出与轻微晃动；上一版把编辑入口同时放进了首页卡片，并让所有类型共享标题修改；大图查看器只保存单张图片；首页摘要优先取最后一篇续写；类型图标仍有部分系统 Emoji 遗留。

**Added**

- 设置页新增 4 套内嵌皮肤：烟熏琥珀、纸页原色、墨色杂志、苔藓书房；选择写入本地偏好，刷新后自动恢复
- 图片查看器新增上一张/下一张、张数提示、键盘方向键与手机左右滑动，并在同一篇章图片间循环浏览
- 日记详情标题支持直接点击进入原位编辑，回车或失焦保存，Escape 取消

**Changed**

- 首页与时间线不再显示编辑图标；篇章编辑入口只在详情页出现，记录级菜单仅保留删除
- 电影、书籍及其他非日记记录的标题恢复只读
- 首页卡片摘要固定展示最初记录，不再被最后一次续写覆盖
- 时间线筛选、收藏标签、空状态、年度回顾、设置统计与新建类型选择统一采用自绘线性 SVG 图标

**Fixed**

- 手机端时间线卡片宽度改为扣除左侧轴线空间，并限制各层容器不横向溢出，消除左右晃动
- 修复音乐、游戏、地点收藏为空时仍渲染空网格、看不到空状态的问题

#### 实测

- `node --check app.js`、`node --check src/features/memory-detail.js` 通过
- `bash tests/run-all.sh`：全部单元/集成测试通过
- 相册循环索引、左右滑动方向与阈值逻辑单测通过；手机视口实测：时间线无横向晃动、首页无编辑图标、日记标题可原位修改、4 套皮肤切换及刷新保持均正常

### V1.24.0 手机端记忆详情与日记编辑体验（2026-09-19）

> 根因：日记保存时强制截取正文前 18 个 UTF-16 字符作为标题；详情页图片只绑定了不存在的 `openViewer`；旧 Sports 历史栈变量迁移后仍被 `popstate` 引用，导致手机返回触发 `ReferenceError`；续写用 `#1/#2` 与密集的小按钮呈现，触屏体验较差。

**Added**

- 新增 `src/features/memory-detail.js`：集中提供电影、书籍、音乐、日记等线性 SVG 图标、Unicode 字素安全截断、日记兜底标题与篇章标签
- 每张记录卡与详情页右上角新增统一编辑入口，可修改标题或删除记录；每篇续写也改为单一编辑入口，再选择修改或删除
- 新增全屏图片查看器，支持点击图片放大、点击遮罩关闭，并接入浏览器返回层级
- 日记新建表单新增独立标题输入；已有日记可在详情编辑菜单中修改标题

**Changed**

- 续写详情由 `#1/#2` 改为「初记 / 续写 1」，使用独立篇章卡片；时间戳移至正文底部，降低视觉干扰
- 时间线及记录卡类型图标换为统一描边 SVG，不再依赖风格不一致的系统 Emoji 图标
- 用户输入统一做 NFC Unicode 归一化，日记兜底标题按完整字素截断，避免手机 Emoji 被截成无效字符
- 手机详情页新增左上角返回按钮；详情打开后隐藏同位置的侧栏按钮，避免重叠

**Fixed**

- 删除 `popstate` 中已迁走的 `tournamentOpenKey/openTournamentDetail` 遗留引用，修复手机返回键无法退出详情页
- 补齐此前缺失的 `openViewer/closeViewer` 实现，修复记录图片无法点击查看大图

#### 实测

- `node --check app.js`、`node --check src/features/memory-detail.js` 通过
- `bash tests/run-all.sh`：14 套单元/集成测试全部通过
- 440px 手机视口实测：独立标题与 Emoji 保存显示、记录/篇章编辑菜单、续写布局、标题编辑弹层、返回关闭弹层及详情返回列表均正常
...
- **根因一（本轮致命）·`.face` 零高裁剪**：翻转卡容器 `.tile .face` 原为 `position:relative; width:100%`，而其两个子面 `.front/.back` 都是 `position:absolute`——绝对定位子元素**不撑开父高度**，`.face` 高度坍缩为 0；它又带 `overflow:hidden`，inline 高 176px 的海报整块被裁剪成 0 像素。**自 V2.0 起照片墙/海报墙每块 tile 实际渲染为不可见空块**（只剩 1px 边框线），换真实图、内嵌 base64 全都被裁掉。修复：`.face` 改 `position:absolute; inset:0` 填满带 inline 高度的 `.tile`；`.back`（翻面记忆卡）同步恢复尺寸。
- **根因二·墙布局失效**：CSS 写 `#wall .cam {...}` 但 HTML 无 `#wall` 元素，`.cam` 取景框尺寸/居中规则整段失效。已改挂真实存在的 `#s5, #s6`。
- **根因三·图片必然 404**：海报走 `assets/promo/*.jpg` 相对路径，部署缺目录/旧缓存/单独拷文件即全失败。已 30 张素材 base64 内嵌（`IMGS` 映射 + 7 处静态标签 data URI），页面 100% 自包含，双击 file:// 直开。
- 顺手修复 3 处海报张冠李戴（沙丘 2/星际穿越/记忆碎片错位互换）。
- 排查方法沉淀：`node --check` 只验语法；本轮的 DOM 桩模拟器（虚拟时钟 + 选择器引擎 + 场景轨迹日志）可复用为宣传页回归工具。

#### 实测
- 模拟器：两份文件各 5509 帧（90s）零崩溃；场景轨迹 s0→s1→tl×3→s5(29s)→s6(35s)→s7→s8→s10→s11→s12→s13→s14 与分镜一致
- `IMGS` 30 键全为有效 data URI；`PH_FILES`/`posters`/`MEMOS` 引用零缺失；`.rth`(46×62)/`.big`(170×230) 显式尺寸不受此 bug 影响
- CSS 布局核算：桌面墙 5 列×745px ≤ `.cam` 780px、竖版 3 列×1599px ≈1560px，均在取景框内
- 视觉终验由用户浏览器完成（模拟器不渲染 CSS）

### 宣传动画 V2.1：海报墙/照片墙真实素材 + 手机竖版 `promo-v2-mobile.html`（2026-08-31）

用户反馈：两墙用渐变+emoji 占位**无代入感**，且需要手机端画面。本轮全部换真实素材并新增竖版。

- **真实素材落地 `assets/promo/`（30 张，共约 1.6MB）**：电影海报/书籍封面走豆瓣 suggest 真实 CDN 图（下载带 Referer+UA；`img9` 节点 418/空响应时按 `server.py` 同款 CDN 节点回退策略换 `img2` 重下）；音乐专辑封面走 iTunes CN 600×600；照片墙 12 张生活照为 AI 生成（随手拍质感：晨跑/海边/咖啡/夜景/骑行/演唱会/猫/花市/山顶云海/老街/火锅/雨窗），Pillow 压缩 640px/JPEG q82
- **两墙换真实图**：照片墙/海报墙 tile 正面 `<img>`（object-fit:cover），海报墙翻面后仍为记忆卡（标题/作者/评分）；`onerror` 回退原渐变+emoji（图丢失不空白）；媒体三连的搜索缩略图与详情大图、时间线卡片封面同步换真实图（地点/日记/事件保留 emoji，贴近真实 app 无封面形态）
- **新增 `promo-v2-mobile.html`（1080×1920 竖版）**：15 场景结构/音乐引擎/控制逻辑与横版一致，布局全面竖版化——两墙 masonry 5 列→3 列、聚合窗口与收束轨道半径按竖版视口收紧（散落 ±420→±300、轨道 560→410）、时间线上下卡距拉大、S1 外壳窄侧栏（隐英文小字）+ 2×2 大卡、洞察 4 列→2 列、HUD 贴底通栏（章节横向滚动、触控按钮 52px）、fit/stageScale/trackX 范围全部按 1080×1920 重算
- 素材全部本地化：双击 file:// 直接可看（不依赖 /img 代理与网络），线上走 Pages CDN

#### 实测
- 两份文件内嵌 JS `node --check` 通过；残留桌面尺寸扫描无（仅音频频率/延时毫秒误报）
- 豆瓣 12 张全部下载成功（2 张经节点回退）；iTunes 6 张 HEAD 200；AI 12 张压缩后共 797KB
- 视觉效果由用户浏览器实测（本机无 GUI）

### 宣传动画 V2：90 秒实时交互产品电影 `promo-v2.html`（2026-08-31）

按用户提供的《InnerOS_90秒宣传动画网页V2_脚本执行器.docx》分镜重做。V1（`promo.html`，竖版 50 秒）本轮已补 Web Audio 音效；V2 为全新文件：1600×900 横版、15 个微场景、约 90 秒，全程**实时 DOM 动画**（非 MP4、非 PPT），让真实产品 UI 在页面里连续运动、切换、聚合。

- **15 微场景全链路**：开场→进入系统→时间线三段（自动滚动 / 多类型卡片 / 真实拖动+惯性）→照片墙（1→3→9→整墙 masonry 错落+镜头缓推）→海报墙（3D 翻转化墙）→媒体三连（搜索→详情→保存，电影/书籍/音乐连续切换）→体育（主队→今天/未来→切 CS2）→资源聚合（散落窗口吸入 InnerOS）→AI 助手→个人洞察→记忆收束（六类图标轨道式汇聚到 Logo）→终章（拉远+音乐淡出）
- **控制系统**：空格播放/暂停、←/→ 切场景、M 静音、R 重播；章节按钮跳转、进度条点击拖跳、时间码显示；鼠标静止 2.6 秒自动隐藏 HUD
- **音乐引擎**：Web Audio 现场合成（pad 底 + 琶音 + 打字/保存/切换等 UI 轻音效），**默认静音**、开启后约 12% 音量、终章自动淡出；场景跳转音乐同步重定位（`resetMusicTo`）
- **视觉**：复用主站设计语言（暗色米金、噪点+暗角、玻璃拟态卡片）；letterbox 缩放自适应任意屏幕；移动端隐藏章节条
- **稳定性**：暂停时 `animation-play-state: paused` 冻结 CSS 动画防失步；场景切换前 reflow 重播动画防残留；媒体三连每次 enter 前克隆节点重置打字动画
- 与正式业务完全隔离：演示数据只存在于宣传页内，不触碰 Memory/IndexedDB/D1

#### 实测
- `node --check` 内嵌 JS 语法通过；12 个场景容器（s0/s1/tl/s5…s14）与 15 条场景表记录一一对应
- 逐场景逻辑走查：时间线三段相位切换、墙 stagger、体育双 enter、收束轨道半径收缩、终章淡出均绑定 tick 主时钟
- 视觉/音效实测由用户浏览器完成（本机无 GUI 预览环境）
...
- **feat：赛事层级重构**——不再直接显示今天/明天/后天混合赛程。改为：
  - 足球首页：⭐我的主队区域 + 🏆热门联赛卡片网格（英超/西甲/德甲/意甲/法甲/欧冠/欧联/中超）
  - 点击联赛 → 进入该联赛赛程页（今天/明天/后天 tab 切换）
  - 点击主队 → 进入主队赛程页（即将开始 + 最近战绩）
  - CS2 首页：⭐我的主队 + 🎮全部赛事（按赛事分组，中文赛事名）
- **feat：主队功能恢复**——可自由添加/删除主队（复用 app.js 球队选择器），点击主队进入专属赛程页，支持足球和 CS2
- **feat：CS2 比赛中文名**——建立常见战队/赛事中文映射表（NAVI/FaZe/G2/Vitality/Spirit 等 40+ 战队，BLAST/IEM/ESL/PGL Major 等赛事），方便查看
- **feat：比赛详情页**——点击任意比赛卡片进入详情页：
  - 足球：双方队徽+队名+比分+状态+时间+场馆，比赛事件（进球/红黄牌/换人时间线），双方阵容（阵型+首发11人）
  - 后端新增 `GET /api/v1/football/fixture?id=xxx`（代理 API-Football，缓存 120s）
  - CS2：详情页占位（地图比分/选手数据开发中）
- **Changed**：`src/features/sports.js` 完整重写（25KB），内置页面状态管理（首页/联赛页/主队页/详情页 + 返回栈），app.js 仍为 4 行薄委托
- **未改**：D1 schema / IndexedDB v4 / 同步逻辑 / 其他页面。

#### 实测
- `node --check`：app.js / sports.js / fixture.js 全部通过
- 逻辑验证：足球首页→联赛→赛程→详情页导航链完整；主队添加/删除/进入赛程页正常；CS2 战队/赛事中文名映射生效
