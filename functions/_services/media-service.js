...
export function createMediaService({ providers }) {
  ...
  function pickProvider(type) {
    ...
    if (!name) throw businessError('VALIDATION_ERROR', '暂不支持的媒体类型：' + type, 400);
    const p = providers[name];
    if (!p) throw businessError('PROVIDER_ERROR', '该类型的媒体服务未配置', 503, true);
    return p;
  }
  ...
  return {
    ...
    async searchMedia({ type, query }) {
      ...
      if (!q) throw businessError('VALIDATION_ERROR', '搜索词不能为空', 400);
      ...
      try {
        ...
        throw businessError('PROVIDER_ERROR', '媒体服务暂时不可用，请稍后重试', 502, true);
      }
    },

    // 用例：媒体详情（标准结构输出）
    async getDetail({ type, id }) {
      if (!id) throw businessError('VALIDATION_ERROR', '缺少 id', 400);
      const provider = pickProvider(type);
      try {
